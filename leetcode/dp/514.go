...
func FindRotateSteps(ring string, key string) int {
	...
	dfs = func(i, j int) (res int) {
		...
		for _, po := range pos[key[j]] { // scan all the position key[j] in ring
			res = min(
				res,
				dfs(po, j+1)+min(abs(i-po), m-abs(i-po)),
			) // we need to make po at 12 o'clock by rotating ring clockwise or counterclockwise (choose the smaller one), and then plus dfs(po,j+1)
		}
		return
	}
